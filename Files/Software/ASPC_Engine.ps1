...
Function updateChecker {
        Write-Host "Running update check"
        Write-Host "Checking to see if its the correct day to run update check"
...
Function Engine_LoadCreds_Login {
    ...
    Write-Output "Creating Credential"
        $AppIDLoc = Get-Content "C:\Program Files\Enigma-Tek\ASPC\Creds\sp_$env:UserName.txt"
        
        $AppTenant = Get-Content "C:\Program Files\Enigma-Tek\ASPC\Creds\spTenant_$env:UserName.txt"
        
        
        $Thumb = "6801F4E6A1B46AB20C3BD2EA27820AF70DFC6B1D"
    
    Write-Output "Credential created"
    Write-Output "Trying to Connect to Azure"
        try {
            Connect-AzureAD -ApplicationId $AppIDLoc -TenantId $AppTenant -CertificateThumbprint $Thumb -Verbose
...
Function Mail_Handler_Primary { 
     ...
     $encPass = Get-Content "C:\cred_$env:UserName.txt" | ConvertTo-SecureString
                $mailuser = 'iaa@crisphealth.org'
                $credential = New-Object System.Management.Automation.PSCredential ($mailuser,$encPass)
        ...
        $mailParams =@{
            ...
            From                = 'iaa@crisphealth.org'
            To                  = 'mike.carey@crisphealth.org'
...
Function Mail_Handler_Secondary {
           ...
           $mailParamsXmail =@{
            ...
            Credential          = $credential
            From                = 'iaa@crisphealth.org'
            To                  = $Global:ExternalMailAddr
            Subject             = 'CSS - Expiring Connection Information'
...
#Start Engine Transcription / log
    ...
    Write-Output "Calling first function: PullEngineConfigs"
    PullEngineConfigs